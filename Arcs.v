...
Section ARC.
  ...
  Definition A_set_eq_commut := U_set_eq_commute Arc.

  Definition A_set_diff_commut := U_set_diff_commute Arc.
  ...
  Definition A_inter_commut := Inter_commute Arc.
  ...
  Fixpoint A_in_neighborhood (x : Vertex) (la : A_list) {struct la} : V_list :=
    ...
    | nil => V_nil
    | A_ends x' y' :: la' =>
      if V_eq_dec x y' then x' :: A_in_neighborhood x la'
      else A_in_neighborhood x la'
    end.

  Fixpoint A_out_neighborhood (x : Vertex) (la : A_list) {struct la} : V_list :=
    match la with
    | nil => V_nil
    | A_ends x' y' :: la' =>
      if V_eq_dec x x'
